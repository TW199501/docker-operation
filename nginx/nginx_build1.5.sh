...
need_install=0; for b in gcc make git curl wget; do command -v "$b" >/dev/null || need_install=1; done
if [ "$need_install" -eq 1 ]; then
  if command -v apt-get >/dev/null 2>&1; then
    ...
    echo "!! 無套件管理器，請手動安裝依賴"; exit 1
  fi
fi

command -v pcre2-config >/dev/null 2>&1 || command -v pcre-config >/dev/null 2>&1 || { echo "!! 缺 PCRE/PCRE2"; exit 1; }

# ===== 內建 PCRE2/OpenSSL 原始碼（交叉編譯）=====
echo ">> 下載 PCRE2/OpenSSL"
...
# ===== 模組載入檔 =====
echo ">> 寫入 /etc/nginx/modules.d/00-load-geoip2.conf"
...
if nginx -t; then
  ...
  echo "[OK] GeoIP2 + CF real_ip 更新完成"
else
  echo "[WARN] nginx -t 失敗，未重載"
  exit 1
fi
...
if [ "$UFW_BASELINE" = "yes" ]; then
  if ! command -v ufw >/dev/null 2>&1; then
    if command -v apt-get >/dev/null 2>&1; then
      echo ">> 未偵測到 UFW，正在安裝..."
      sudo apt-get update -y && sudo apt-get install -y ufw || { echo "!! UFW 安裝失敗，跳過 UFW 基線"; UFW_BASELINE="no"; }
      # 開啟 IPv6 管理（若存在設定檔）
...
if [ "$UFW_BASELINE" = "yes" ] && command -v ufw >/dev/null 2>&1; then
  ...
  if [ "$UFW_SSH_LIMIT" = "yes" ]; then
    mapfile -t del_idx < <(sudo ufw status numbered | awk -v p="22/tcp" -v c="$LAN_CIDR" '$0 ~ /^\[/ && $2==p && index($0,c)>0 { gsub(/[\[\]]/,"",$1); print $1 }' | sort -nr)
