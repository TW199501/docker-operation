...
# 版本：v1.6（整合 UFW 基線；update 腳本可選同步 UFW）
# 說明：編譯 Nginx 1.29.3，開啟 UFW + GeoIP2 + Cloudflare real_ip
# 日期：2025-11-24
# ===============================================
set -euo pipefail
...
# 確保主設定會載入 modules/*.conf（放在檔案最上面最穩）
...
# （原本這裡有模組加載前 nginx -t，自動回寫 nginx.conf，已整段停用）
# echo ">> 檢查 Nginx 配置（模組加載前）..."
# if ! $SUDO nginx -t >/dev/null 2>&1; then
#   echo ">> 檢測到 nginx 配置有誤，回寫標準 nginx.conf ..."
#   echo ">> 這可能是因為模組尚未加載，在首次安裝時是正常的"
#   write_base_nginx_conf
# fi
...
if [ "$UFW_SYNC" = "1" ] && command -v ufw >/dev/null 2>&1; then
  ...
  del_net() {
    local n="$1" p="$2"
    mapfile -t idx < <("$UFW_BIN" status numbered | awk -v n="$n" -v p="$p/tcp" '$0 ~ /^\[/ && $2==p && index($0,n)>0 { gsub(/[\[\]]/,"",$1); print $1 }' | sort -nr)
    for i in "${idx[@]:-}"; do
      "$UFW_BIN" --force delete "$i" || true
    done
  }
  ...
  for n in $CF4_ADD; do
    for p in 80 443; do
      "$UFW_BIN" allow proto tcp from "$n" to any port "$p" comment "cf-auto"
    done
  done
  for n in $CF6_ADD; do
    for p in 80 443; do
...
# 4) 重載 nginx：不再執行 nginx -t，只做 reload
CFG=""
...
if [ -n "$CFG" ]; then
  if nginx -s reload; then
    ...
    echo "[WARN] reload with -c $CFG 失敗，請手動檢查 nginx"
  fi
elif [ -s /run/nginx.pid ] && kill -0 "$(cat /run/nginx.pid)" 2>/dev/null; then
  if nginx -s reload; then
    echo "[OK] reload（default conf）"
  else
    echo "[WARN] reload（default conf）失敗，請手動檢查 nginx"
  fi
else
...
# ===== 最終驗證並套用（已停用 nginx -t，只保留 apt-mark）=====
